...
<script>
  export default {
    ...
    methods: {
      init (wjlx, wjlxdm, uid) {
        this.uid = uid
        this.wjlx = wjlx
        this.wjlxdm = wjlxdm
        this.fileList = []
        this.visible = true
        this.url = this.$http.adornUrl(`/shopjoin_manage/joinfile/joinfiles?token=${this.$cookie.get('token')}`)
        // 上传文件的时候向后台传参
      ...
      success (response, file, fileList) {
        ...
        this.$http({
          // 上传成功后，向数据库存数据
          url: this.$http.adornUrl(`/shopjoin_manage/joinfile/joinxdsave`),
          method: 'post',
